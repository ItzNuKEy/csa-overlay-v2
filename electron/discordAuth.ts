...
// Works in both development and production (if .env is in the app directory)
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const envPath = path.resolve(__dirname, "..", ".env");
// Only load if file exists (dotenv won't error if missing, but this is explicit)
if (fs.existsSync(envPath)) {
  config({ path: envPath });
} else {
  // Try loading from current working directory (for development)
  config();
}
