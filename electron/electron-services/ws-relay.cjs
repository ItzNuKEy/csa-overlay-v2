...
function start() {
  // your current file’s contents move into here
  // keep your hard-coded 49322 and localhost:49122
  // but store the interval in reconnectInterval

  return {
    stop() {
      try { reconnectInterval && clearInterval(reconnectInterval); } catch {}
      reconnectInterval = null;

      try { wsClient && wsClient.close(); } catch {}
      wsClient = null;
      ...
      try {
        Object.keys(connections).forEach((k) => {
          try { connections[k].connection.close(); } catch {}
